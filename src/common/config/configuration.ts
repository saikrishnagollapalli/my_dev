export default () => ({
  app: {
    ...
    health: {
      enablePeriodicCheck:
        process.env.ENABLE_PERIODIC_HEALTH_CHECK === 'true' || false,
      disk: {
      ...
      memory: {
        ...
        heapThreshold:
          parseInt(process.env.MEMORY_HEALTH_HEAP_THRESHOLD, 10) ||
          150 * 1024 * 1024,
        rssKey: process.env.MEMORY_HEALTH_RSS_KEY || 'memory_rss',
        rssThreshold:
          parseInt(process.env.MEMORY_HEALTH_RSS_THRESHOLD, 10) ||
          150 * 1024 * 1024,
      },
    },
    error: {
